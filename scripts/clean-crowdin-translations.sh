...
rm -f assets/fabled_roots/lang/en_US.json
cp /tmp/fabled_roots-en_us.json assets/fabled_roots/lang/en_us.json

# Crowdin exports locale names like de_DE.json, but Minecraft lang files are lowercase.
find assets/fabled_roots/lang -maxdepth 1 -type f -name "*.json" | while read -r file; do
  dir="$(dirname "$file")"
  base="$(basename "$file")"
