...
function getTemplatePoolFile(poolId) {
  const [namespace, poolPath] = poolId.includes(":")
    ? poolId.split(":")
    : ["minecraft", poolId];
...
function getStructureNbtFileFromLocation(location) {
  const [namespace, structurePath] = location.includes(":")
    ? location.split(":")
    : ["minecraft", location];
...
async function main() {
  ...
  if (worldgenFiles.length > 0) {
    for (const worldgenFile of worldgenFiles) {
      ...
      const outputPath = path.join(
        ...
        group.namespace,
        "structure",
        `${group.relativePath}${outputExtension}`
      );

      validOutputFiles.add(path.normalize(outputPath));

      fs.mkdirSync(path.dirname(outputPath), { recursive: true });
      fs.writeFileSync(outputPath, await generateMarkdown(group.relativePath, structures, totals));
