...
execute if entity @e[type=player,tag=fabled_roots.dunesworn,distance=1..32] if items entity @s armor.* #fabled_roots:custom_item_base[minecraft:custom_data~{fabled_roots:{stronger_together: 1b}}] run effect give @e[type=player,tag=fabled_roots.dunesworn,distance=..32] minecraft:strength 2 1 true
...
execute if predicate fabled_roots:entity/is_crouching run effect give @e[distance=0.1..4] minecraft:hunger 2 0 false
