...
execute if items entity @s weapon.mainhand minecraft:lapis_lazuli run scoreboard players add @s fabled_roots.bartering.timer 1
execute \
    if score @s fabled_roots.bartering.timer matches 5.. \
...
$execute if score @s fabled_roots.bartering.timer matches 1 run return run playsound fabled_roots:entity.descendant.encourage ambient @a ~ ~ ~ 0.5 $(pitch)
