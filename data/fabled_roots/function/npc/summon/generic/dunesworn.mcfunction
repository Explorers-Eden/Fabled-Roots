...
execute if score $model fabled_roots.technical matches 1..2 store result storage eden:temp fabled_roots.npc.id int 1 run random value 1..13
...
data modify storage eden:temp fabled_roots.npc.race set value "dunesworn"
data modify storage eden:temp fabled_roots.npc.race_cap set value "Dunesworn"

execute store result storage eden:temp fabled_roots.npc.size float 0.01 run random value 85..95
