advancement revoke @s only fabled_roots:consumed_enchanted_scroll/lupine_echo

execute store result score @s fabled_roots.exp.player.level run experience query @s levels
execute if score @s fabled_roots.exp.player.level matches ..29 run return run function fabled_roots:item/enchanted_scroll/cancel {scroll:"lupine_echo"}
execute unless entity @e[type=#fabled_roots:scroll_targets,distance=1..24] run return run function fabled_roots:item/enchanted_scroll/cancel {scroll:"lupine_echo"}
...
execute as @n[type=#fabled_roots:scroll_targets,distance=1..24] at @s run function fabled_roots:item/enchanted_scroll/lupine_echo/exec