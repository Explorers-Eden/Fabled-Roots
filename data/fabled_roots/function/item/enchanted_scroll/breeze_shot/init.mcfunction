advancement revoke @s only fabled_roots:consumed_enchanted_scroll/breeze_shot

execute store result score @s fabled_roots.exp.player.level run experience query @s levels
execute if score @s fabled_roots.exp.player.level matches ..29 run return run function fabled_roots:item/enchanted_scroll/cancel {scroll:"breeze_shot"}
...
function fabled_roots:item/enchanted_scroll/breeze_shot/exec with storage fabled_roots:temp enchanted_scroll